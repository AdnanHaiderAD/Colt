...
public class TernarySearchTrie {
	...
	/**Author:Adnan 
	 * The default constructor has been modified such that now  it constructs a Ternary Search Trie and loads data from a <code>File</code> into the Trie. 
	 *  The file is a normal text document, where each line  is a word instead of reading word:value pair".
	 *
	 *@param  file              The <code>File</code> with the data to load into the Trie.
	...
	public TernarySearchTrie(File file, boolean compression) throws IOException {
		...
		while ((word = in.readLine()) != null) {
			numWords++;
			word= word.trim();
			//if the line is just empty skip to the next line : author -Adnan;
			...
			if (key.length() > 0 && rootNode != null) {
				...
				while (true) {
					...
					if (charComp == 0) {
						...
						//author :Adnan -instead of calling getOrCreateNode that starts reading from the root again..nodes are created here to improve insertion time
				...
				//currentNode =getOrCreateNode(StringUtils.toLowerCase(word.trim(), false));
